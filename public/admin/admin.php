<?php
/**
 * public/admin/admin.php  —  TD Rentals administration panel
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add_vehicle') {
        ...
        if (!$name || !$price) {
            ...
            // Upload image into assets/images/ (project root level)
            $imageFile = uploadVehicleImage($_FILES['image'] ?? []);
            if (!$imageFile) $imageFile = 'hero-porsche.jpg';

            $stmt = db()->prepare("INSERT INTO cars
                (name,subtitle,price_per_day,tag,image_file,top_speed,acceleration,max_power,engine,description1,description2,available,is_featured)
                VALUES (?,?,?,?,?,?,?,?,?,?,?,1,?)");
            $stmt->execute([$name,$subtitle,$price,$tag,$imageFile,$topspeed,$accel,$power,$engine,$desc1,$desc2,$featured]);
            $newId = (int)db()->lastInsertId();
            auditLog("vehicle_added", "car", $newId, "Added: {$name}");
            setFlash('success', "Vehicle '{$name}' added successfully.");
        }
    ...
    if ($action === 'edit_vehicle') {
        ...
        if (!$name || !$price) {
            ...
            $params[] = $cid;
            db()->prepare("UPDATE cars SET name=?,subtitle=?,price_per_day=?,tag=?,description1=?,description2=?,
                           top_speed=?,acceleration=?,max_power=?,engine=?,is_featured=?,available=?
                           {$imgSql} WHERE id=?")->execute($params);
            auditLog("vehicle_edited", "car", $cid, "Edited: {$name}");
            setFlash('success', "Vehicle '{$name}' updated.");
        }
        header('Location: admin.php?page=fleet'); exit;
    }
    ...
    if ($action === 'delete_vehicle') {
        ...
        if ($active->fetchColumn() > 0) {
            ...
            $row = db()->prepare("SELECT name FROM cars WHERE id = ?");
            $row->execute([$cid]);
            $carName = ($row->fetch())['name'] ?? 'unknown';
            db()->prepare("DELETE FROM cars WHERE id = ?")->execute([$cid]);
            auditLog("vehicle_deleted", "car", $cid, "Deleted: {$carName}");
...
$activePerCar = [];
foreach (db()->query("SELECT car_id, COUNT(*) AS cnt FROM bookings
                      WHERE status IN ('pending','confirmed') GROUP BY car_id")->fetchAll() as $r) {
...
$revenueByDay = [];
try {
    $rows = db()->query("SELECT DATE(created_at) AS d, SUM(grand_total) AS rev
...
:root{
  ...
  --border:#2a2a2a;--border2:#333;--fg:#f0f0f0;--fg2:#999;--fg3:#666;
  --red:#e03535;--red2:#c02020;--green:#22c55e;--yellow:#f59e0b;--blue:#3b82f6;
  --font:'Inter',sans-serif;--display:'Bebas Neue',sans-serif;
  --sidebar:220px;--radius:6px;
}
html,body{height:100%;background:var(--bg);color:var(--fg);font-family:var(--font);font-size:14px;line-height:1.5}
a{color:inherit;text-decoration:none} img{display:block;max-width:100%} button{font-family:var(--font);cursor:pointer}
...
.sb-brand-name{font-family:var(--display);font-size:1.4rem;letter-spacing:.05em}
...
.sb-item{display:flex;align-items:center;gap:.75rem;padding:.65rem 1.25rem;font-size:.75rem;font-weight:500;letter-spacing:.04em;text-transform:uppercase;color:var(--fg2);transition:.15s;border-left:3px solid transparent}
.sb-item:hover{color:var(--fg);background:var(--bg3)} .sb-item.active{color:var(--fg);background:var(--bg3);border-left-color:var(--red)}
...
.sb-uname{font-size:.8rem;font-weight:600} .sb-urole{font-size:.65rem;color:var(--fg3);text-transform:uppercase;letter-spacing:.06em}
.sb-link{display:flex;align-items:center;gap:.5rem;font-size:.7rem;color:var(--fg3);padding:.35rem 0;transition:.15s}
.sb-link:hover{color:var(--fg2)} .sb-link svg{width:13px;height:13px}
/* MAIN */
...
.search-box input::placeholder{color:var(--fg3)} .search-box svg{width:14px;height:14px;color:var(--fg3);flex-shrink:0}
...
.icon-btn:hover{background:var(--bg4)} .icon-btn svg{width:16px;height:16px;color:var(--fg2)}
...
.btn-red{background:var(--red);color:#fff;padding:.45rem 1rem} .btn-red:hover{background:var(--red2)}
...
tr:last-child td{border:none} tr:hover td{background:rgba(255,255,255,.02)}
/* BADGES */
...
.b-confirmed{background:rgba(34,197,94,.12);color:#4ade80} .b-pending{background:rgba(245,158,11,.12);color:#fbbf24}
.b-cancelled{background:rgba(224,53,53,.12);color:#f87171} .b-completed{background:rgba(59,130,246,.12);color:#60a5fa}
.b-available{background:rgba(34,197,94,.12);color:#4ade80}  .b-hidden{background:rgba(224,53,53,.12);color:#f87171}
.b-featured{background:rgba(59,130,246,.12);color:#60a5fa}  .b-upcoming{background:rgba(245,158,11,.12);color:#fbbf24}
.b-verified{background:rgba(34,197,94,.12);color:#4ade80}   .b-rejected{background:rgba(224,53,53,.12);color:#f87171}
/* MINI CHART */
.mini-chart{display:flex;align-items:flex-end;gap:3px;height:50px;margin-top:.75rem}
.mini-bar{flex:1;background:var(--bg4);border-radius:2px 2px 0 0;min-height:4px} .mini-bar.hi{background:var(--red)}
/* FLEET GRID */
...
.dot{width:8px;height:8px;border-radius:50%;background:var(--bg4)} .dot.on{background:var(--red)}
...
.filter-tab:hover{color:var(--fg)} .filter-tab.on{background:var(--red);border-color:var(--red);color:#fff}
.action-icon{background:none;border:none;color:var(--fg3);cursor:pointer;padding:.2rem;transition:.15s;display:inline-flex}
.action-icon:hover{color:var(--fg)} .action-icon svg{width:15px;height:15px}
/* MODAL */
...
.fleet-overview-card{background:var(--bg2);border:1px solid var(--border);border-radius:var(--radius);padding:1.5rem}
...
.verif-name{font-size:.82rem;font-weight:600} .verif-role{font-size:.65rem;color:var(--fg3);text-transform:uppercase;letter-spacing:.06em}
...
<aside class="sidebar">
  ...
  <div class="sb-bottom">
    ...
    <div style="margin-top:.75rem;display:flex;gap:1rem">
      <a href="../user/index.php" class="sb-link">
      ...
      <a href="../authentication/logout.php" class="sb-link">
    ...
    <div class="tbl-wrap">
      <table>
        ...
        <tr>
          <td>
            <div style="display:flex;align-items:center;gap:.75rem">
              <div style="width:44px;height:32px;background:var(--bg4);border-radius:4px;overflow:hidden;flex-shrink:0">
                <img src="../../assets/images/<?= htmlspecialchars($b['image_file'] ?? 'hero-porsche.jpg') ?>"
          ...
          <td style="font-weight:600">NPR<?= number_format($b['grand_total'], 2) ?></td>
        </tr>
        <?php endforeach; ?>
        </tbody>
      </table>
  ...
  <div class="fleet-grid">
    ...
    <div class="fleet-card">
      <div class="fleet-card-img">
        <img src="../../assets/images/<?= htmlspecialchars($c['image_file']) ?>" alt=""
      ...
      <div class="fleet-card-body">
        ...
        <div class="fleet-card-actions">
          ...
          <button class="btn btn-outline-red" style="flex:1"
                  ...
                  onclick="confirmDelete(<?= $c['id'] ?>,'<?= htmlspecialchars($c['name'], ENT_QUOTES) ?>')">
...
<div class="content">
  ...
  <div class="two-col">
    <div class="sec">
      <div class="sec-head"><span class="sec-title">Geographic Hubs</span></div>
      <div class="sec-body">
        <?php foreach ([['Los Angeles',42],['New York',28],['Miami',18],['Las Vegas',12]] as $h): ?>
        <div class="geo-bar">
    ...
    <div class="sec">
      ...
      <div class="sec-body">
        <div class="review-card">
          ...
          <div class="review-text">"The GT3 RS is a beast. TD Rentals delivered it straight to my hotel in Beverly Hills."</div>
    ...
    <div class="tbl-wrap">
      <table id="bookings-table">
        ...
        <tr data-status="<?= $b['status'] ?>">
          ...
          <td><span class="badge b-<?= $b['status'] ?>"><?= ucfirst($b['status']) ?></span></td>
          <td style="font-weight:700">NPR<?= number_format($b['grand_total'], 2) ?></td>
          <td>
...
<footer class="admin-footer">TD RENTALS &copy; <?= date('Y') ?> — Engineered for Performance</footer>
</main>
</div>
